...
- **Cross-Midnight Shift Handling**: Day type (full/half/off) and Saturday cap calculated from working date (not current calendar day), so shifts crossing midnight use correct day's rules. Break times and counts for two-shift users are filtered by active shift period (morning/evening), not combined. Dashboard auto-switches to the active shift tab. Target hours shown per-period for two-shift users.
- **9 AM Day Reset (Admin Dashboard)**: `getTodayShifts()` uses 9 AM PKT as the working-day boundary. Before 9 AM, the working date is set to yesterday so the admin Staff Monitor still shows the previous day's shifts until the new day starts. Cross-midnight active shift inclusion is maintained.
- **Admin Two-Shift Display**: Admin dashboard employee cards show separate morning and evening rows for two-shift employees, each with independent status (Not Started/Working/Completed), clock-in/out times, work hours, and break counts. One-shift and open employees show a single detail row.
